...
func (m Mutations) Augment(s *ast.Schema) error {
	...
	for _, v := range s.Types {
		...
		if c, ok := args["create"]; ok && cast.ToBool(c) {
			s.Mutation.Fields = append(s.Mutation.Fields, m.addCreateMutation(s, def))
		}
		if c, ok := args["delete"]; ok && cast.ToBool(c) {
			s.Mutation.Fields = append(s.Mutation.Fields, m.addDeleteMutation(s, def))

		}

		if c, ok := args["update"]; ok && cast.ToBool(c) {
			s.Mutation.Fields = append(s.Mutation.Fields, m.addUpdateMutation(s, def))
...
func (m Mutations) addUpdateMutation(s *ast.Schema, obj *ast.Definition) *ast.FieldDefinition {
